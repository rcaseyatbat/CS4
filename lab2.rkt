...
;; a space  complexity of O(log(n)), since we call on n, n/2, n/4, n/8,
;; n/16,...etc until we get n below the tolerance of 1.  With applicative 
...
;; The time complexity is O(n), since we have to go through log(n) 
;; recursive function calls twice.  Note if we start with n as a power of 2, say
;; n = 2^i, we have to make 2^i + 2^(i-1) + ... + 2^0 total calls.  The biggest
;; term of this sum is 2^i, which is equal to n.  Since we divide by 2 each 
;; time, the rest of the terms can add up to at most 2^i.  So, we have a max
;; of 2n function calls, and thus we have time complexity of O(n). Also, we 
;; could multiply in the constant space and time complexities of 1 for the 
;; function "f", but that also wouldn't change anything, as the total 
;; complexities would remain the same.
...
;; Note that lambda shielding takes place here, preventing 7 from being
;; substitute in for the y in the product, as the inner lambda (y) is 
;; responsible for the y in the product.
;;   ((lambda (y)
;;     ((lambda (z)
;;       (* x y z)) 22)) 14))
;;    apply:
...
(define (sum-accum-rec term a next b)
  (accumulate-rec + 0 term a next b))
(define (product-accum-rec term a next b)
  (accumulate-rec * 1 term a next b))

(check-expect (sum-accum-rec square 4 step1 5) 41) ;; sum works
(check-expect (product-accum-rec square 4 step1 5) 400) ;; product works
...
(check-expect (sum-accum-iter square 4 step1 5) 41) ;; sum works
(check-expect (product-accum-iter square 4 step1 5) 400) ;; product works
