            ...
            ((eq? op 'jump-ok?)
             (check-args args 1 "make-square 'jump-ok?")
              (if (is-valid-color? (car args))
                  (jump-help color (car args)) ;; return true if valid jump over
                  (error "not a valid color: " (car args))))
            ((eq? op 'land-ok?)
             (check-args args 1 "make-square 'land-ok?")
              (if (is-valid-color? (car args))
                  (land-help color (car args)) ;; return true if valid land on
                  (error "not a valid color: " (car args))))
            ((eq? op 'jump!)
             (check-args args 1 "make-square 'jump!")
              (if (is-valid-color? (car args))
                  (if (jump-help color (car args))
                      (do-jump! color (car args)) ; helper function to set new color
             ...
             (check-args args 1 "make-square 'land!")
              (if (is-valid-color? (car args))
                  (if (land-help color (car args))
                      (do-land! color (car args)) ; helper function to set new color
...
;; possibilities as opposed to using the internal representation of the 3-element
;; list (which it might have been easier to modify).  But this way, changing
;; the internal representation would only mean changing the helper functionst 
;; that convert a color symbol to a internal representation and vice verse, 
;; which would not be much work at all.
